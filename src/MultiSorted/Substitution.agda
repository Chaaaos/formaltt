...
module MultiSorted.Substitution {ℓ} {Σ : Signature} (T : Theory Σ ℓ) where

  open Signature Σ
  open Theory T
  -- an equality is preserved by the action of the identity
  eq-id-action : ∀ {Γ : Context} {A} {u v : Term Σ Γ A} → (Γ ⊢ (u [ id-substitution ]s) ≈ (v [ id-substitution ]s) ⦂ A) → (Γ ⊢ u ≈ v ⦂ A)
