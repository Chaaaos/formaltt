...
module SecondOrder.SecondOrderTheory {ℓs ℓo ℓa : Level} {𝔸 : Arity} {Σ : SecondOrderSignature.Signature {ℓs} {ℓo} {ℓa} 𝔸}where
  ...
  -- Theory
    ...
    -- terms and judgemental equality form a setoid
      ...
      _≈ʳ_ : ∀ {Γ Δ : Context} {Θ} (σ τ : Θ ⊕ Γ ⇒ʳ Δ) → Set (lsuc (ℓs ⊔ ℓo ⊔ ℓa ⊔ ℓ))
      _≈ʳ_ {Γ} {Δ} {Θ} σ τ = ∀ {A} (x : A ∈ Γ) → ⊢ Θ ⊕ Δ ∥ tm-var (σ x) ≈ tm-var (τ x) ⦂ A

      -- equality of substitutions
      _≈ˢ_ : ∀ {Γ Δ : Context} {Θ} (σ τ : Θ ⊕ Δ ⇒ˢ Γ) → Set (lsuc (ℓs ⊔ ℓo ⊔ ℓa ⊔ ℓ))
      _≈ˢ_ {Γ} {Δ} {Θ} σ τ = ∀ {A} (x : A ∈ Γ) → ⊢ Θ ⊕ Δ ∥ σ x ≈ τ x ⦂ A

      -- equality of metavariable instatiations
      _≈ⁱ_ : ∀ {Γ Θ ψ} (I μ : _⇒ⁱ_⊕_ {Σ = Σ} ψ Θ Γ) → Set (lsuc (ℓs ⊔ ℓo ⊔ ℓa ⊔ ℓ))
      _≈ⁱ_ {Γ} {Θ} {ψ} I μ = ∀ (M : mv Θ) → ⊢ ψ ⊕ (Γ ,, mv-arity Θ M) ∥ I M ≈ μ M ⦂ (mv-sort Θ M)
