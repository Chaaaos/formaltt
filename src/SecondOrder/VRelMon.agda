    ...
    record Codomain-Jⱽ-Elt : Set ((lsuc ℓ)) where
      ...
      field
        ...
        F₁ : ∀ {Θ ψ Δ Ξ} (ρ : Δ ⇒ᵛʳ Ξ) -- (ι : Θ ⇒ᵐʳ ψ)
             → (F₀ Δ Θ) ⇒ (F₀ Ξ ψ)
        identity : ∀ {Θ Δ}
                   →  Category._≈_ (Setoids ℓ ℓ) (F₁ (idᵛʳ {Δ})) (id {F₀ Δ Θ}) -- Category._≈_ (Setoids ℓ ℓ) (F₁ (idᵛʳ {Δ}) (idᵐʳ {Θ})) (id {F₀ Δ Θ})
        homomorphism : ∀ {Θ ψ Ω Γ Δ Ξ} {ρ : Γ ⇒ᵛʳ Δ} {ι : Θ ⇒ᵐʳ ψ} {τ : Δ ⇒ᵛʳ Ξ} {μ : ψ ⇒ᵐʳ Ω}
                       → Category._≈_ (Setoids ℓ ℓ) (F₁ (τ ∘ᵛʳ ρ)) ((F₁ τ) ∘ (F₁ ρ)) -- Category._≈_ (Setoids ℓ ℓ) (F₁ (τ ∘ᵛʳ ρ) (μ ∘ᵐʳ ι)) ((F₁ τ μ) ∘ (F₁ ρ ι))
        F-resp-≈ : ∀ {Θ ψ Γ Δ} {ρ τ : Γ ⇒ᵛʳ Δ} {ι μ : Θ ⇒ᵐʳ ψ}
                   → (ρ ≡ᵛʳ τ) → (ι ≡ᵐʳ μ) → (Category._≈_ (Setoids ℓ ℓ) (F₁ ρ) (F₁ τ))
...
--                      {!!} ≈⟨ {!!} ⟩
--                      {! [ (λ x₂ → var-inl x₂) ]ˢ!} ≈⟨ {! ˢ∘ᵛʳ-ᵛʳ∘ˢ-disjoint ˢ∘ᵛʳ-ᵛʳ∘uˢ-disjoint!} ⟩
...
--                              → begin⟨ Term-setoid Θ _ _ ⟩
--                                       (η (𝒩s i) Θ Γ ⟨$⟩ x) ≈⟨ Setoids≈-⟨$⟩ (η (𝒩s i) Θ Γ) (η (𝒩s i) Ω Γ) {!!} x ⟩
...
--                                               ([ (λ {B} → _⟨$⟩_ (η (k B) Θ Λ)) ]ˢ u) ≈⟨ []ˢ-resp-≈ˢ u (λ x → Setoids≈-⟨$⟩ {! η (k C) Θ Λ!} {!η (h C) Θ!} {!!} x) ⟩
