...
module SecondOrder.VRelMon
  ...
  module _ {Θ : MContext} where
    ...
    VMonad : Monad Jⱽ
    Functor-Jⱽ.F₀ (Monad.F₀ VMonad Γ A) Δ ψ = Term-setoid Θ (Γ ,, Δ) A
    _⟨$⟩_      (Functor-Jⱽ.F₁ (Monad.F₀ VMonad Γ A) ρ ι) = [ ʳ⇑ᵛʳ ρ ]ᵛʳ_
    func-cong (Functor-Jⱽ.F₁ (Monad.F₀ VMonad Γ A) ρ ι) ξ = []ᵛʳ-resp-≈ ξ
    Functor-Jⱽ.identity     (Monad.F₀ VMonad Γ A) ξ = ≈-trans ([]ᵛʳ-resp-≡ᵛʳ idᵛʳ+idᵛʳ) (≈-trans [id]ᵛʳ ξ)
    Functor-Jⱽ.homomorphism (Monad.F₀ VMonad Γ A) {ρ = ρ} {ι = ι} {τ = τ} {μ = μ} {x = x} {y = y} ξ
      = begin⟨ Term-setoid Θ _ _ ⟩
          ...
          [ ʳ⇑ᵛʳ (τ ∘ᵛʳ ρ) ]ᵛʳ y ≈⟨ []ᵛʳ-resp-≡ᵛʳ (λ x₁ →
              uniqueᵛʳ² {τ = [ (λ x₂ → var-inl x₂) , (λ x₂ → var-inr (τ (ρ x₂))) ]ᵛʳ}
                       ...
                       (λ x₂ → refl) (λ x₂ → refl) x₁) ⟩
          [ σ' ]ᵛʳ y ≈⟨ [∘]ᵛʳ ⟩
         ((Functor-Jⱽ.F₁ (Monad.F₀ VMonad Γ A) τ μ ∘
           Functor-Jⱽ.F₁ (Monad.F₀ VMonad Γ A) ρ ι) ⟨$⟩ y)
         ∎
         where
           σ' : (Γ ,, _) ⇒ᵛʳ Γ ,, _
           σ' = [ var-inl
    ...
    commute (Monad.extend VMonad {Υ} {Ω} σ A) {Ξ} {Ψ} {Γ} {Δ} ρ ι {x} {y} x≈y
      = begin⟨ Term-setoid Θ _ _ ⟩
        (η (Monad.extend VMonad σ A) Ψ Δ ∘
          Functor-Jⱽ.F₁ (Monad.F₀ VMonad Υ A) ρ ι
          ⟨$⟩ x)
          ≈⟨ ≈-sym ([ˢ∘ᵛʳ]ˢ x) ⟩
        ...
        (Functor-Jⱽ.F₁ (Monad.F₀ VMonad Ω A) ρ ι ∘
      ...
      where
        ...
        η-ˢ∘ᵛʳ ρ ι (var-inl x) =
                   begin⟨ Term-setoid Θ _ _ ⟩
                     ((λ {B} → _⟨$⟩_ (η (σ B) Θ _)) ˢ∘ᵛʳ (ʳ⇑ᵛʳ ρ)) (var-inl x) ≈⟨ {!!} ⟩
                     {!!} ≈⟨ {!!} ⟩
                     {!!} ≈⟨ ≈-sym {! (ˢ∘ᵛʳ-ᵛʳ∘ˢ-disjoint {!!} {!!} (var-inl x)) !} ⟩
               ...
               where
                 ...
                 ˢ∘ᵛʳ-ᵛʳ∘ˢ-disjoint σ τ (var-inr x) = {!!}
    ...
    Monad.identityʳ VMonad {_} {_} {𝒩s} =
                           λ i {k = Ω} {Γ = Γ} {x = x} {y = y} x≈y
                             → begin⟨ Term-setoid Θ _ _ ⟩
                                      ...
                                      (η (𝒩s i) Ω Γ ⟨$⟩ y) ∎
                    -- where
                    --   η-MCtx-indep : ∀ {Γ Ξ Δ : VContext} {ψ Ω : MContext} {i : sort} {ρ₁ ρ₂}
                    --                  (ρ->σ : (Category._⇒_ Functors-Jⱽ) (Functor.₀ Jⱽ Ξ i) (Monad.F₀ VMonad Δ i))
                    --                    →  (Category._≈_ (Setoids ℓ ℓ) ρ₁ ρ₂)
                    --                    → ( ∀ (x : i ∈ Ξ ,, Γ) → SecondOrder.Term._≈_ Σ ( ρ₁ ⟨$⟩ x) ( ρ₂ ⟨$⟩ x))
                    --   η-MCtx-indep ρ->σ  ξ x = {!!}


-- Goal: (Σ SecondOrder.Term.≈ η ρ-σ ψ Γ ⟨$⟩ x) (η ρ-σ Ω Γ ⟨$⟩ x)
    ...
    Monad.extend-≈ VMonad {Γ} {Δ} {k} {h} = λ ξ A {ψ} {Λ} {t} {u} ξᵗ
                                            → begin⟨ Term-setoid Θ _ _ ⟩
                                              ...
                                              ([ (λ {B} → _⟨$⟩_ (η (k B) Θ Λ)) ]ˢ u) ≈⟨ []ˢ-resp-≈ˢ u {!!} ⟩
